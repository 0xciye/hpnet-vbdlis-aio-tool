...
class DiagnosticLoggerTests(unittest.TestCase):
    ...
    def test_automatic_exclusions_are_logged_and_remaining_people_export(self):
        ...
        result = self.process()
        self.assertTrue(result.can_export)
        self.assertEqual(result.stats["households_skipped"], 1)
        self.assertEqual(result.stats["people_skipped_individually"], 1)
        self.assertEqual(result.stats["people_skipped"], 3)
        self.assertEqual([r["H"] for r in result.rows], ["NGUYỄN VĂN A", "NGUYỄN VĂN D"])
        text = self.text(result.diagnostic_files)
        self.assertIn("ĐÃ BỎ QUA TOÀN BỘ HỘ", text)
